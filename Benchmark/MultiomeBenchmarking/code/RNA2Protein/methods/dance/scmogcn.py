...
train_id = "21"
test_id = "22"
outputs = "../dance/"+test_id+"_from_"+train_id+"_dance.csv"
...
def pipeline(transductive=True, verbose=2, logger=None, **kwargs):
    ...
    # if transductive:
    #     B = np.zeros(true_test_mod2.toarray().shape)
    #     y_test = torch.from_numpy(B).to(device)
    ...
    pd.DataFrame(model.predict(g, np.arange(TRAIN_SIZE, CELL_SIZE)).cpu().numpy().T).to_csv(outputs)
...
if __name__ == '__main__':
    ...
    # pd.DataFrame(model.predict(test_mod1).cpu().numpy().T).to_csv(outputs)
