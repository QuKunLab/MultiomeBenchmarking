...
if __name__ == '__main__':
    ...
    if args.subtask=='openproblems_bmmc_cite_phase2_rna':
        pd.DataFrame(model.predict(test_mod1).cpu().numpy().T).to_csv(args.outdir+"/"+test_id+"_from_"+train_id+"_babel.csv")
    else:
        np.save(args.outdir+"_peak_predict.npy",model.predict(test_mod1).cpu().numpy().T)
