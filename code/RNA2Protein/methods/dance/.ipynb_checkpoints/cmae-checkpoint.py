...
if __name__ == '__main__':
    ...
    if opts.subtask=='openproblems_bmmc_cite_phase2_rna':
        pd.DataFrame(model.predict(test_mod1).cpu().numpy().T).to_csv(output_directory+test_id+"_from_"+train_id+"_cmae.csv")
    else:
        np.save(output_directory+"_peak_predict.npy",model.predict(test_mod1).cpu().numpy().T)
