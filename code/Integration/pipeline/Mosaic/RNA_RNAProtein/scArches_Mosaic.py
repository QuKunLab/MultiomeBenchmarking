...
adata_CITE.obsm['protein_expression'] = adata_ADT.to_df()
...
del X
adata_rna.obsm['protein_expression'] = pd.DataFrame(np.zeros((adata_rna.shape[0],len(protein_name))),
                                                    index=cell_names.cell_ids,
...
np.savetxt(arguments[3]+"/scArches.csv", latent, delimiter=',')
# end_time = time.time()
