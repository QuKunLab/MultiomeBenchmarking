...
atac.var['modality']=['peak']*atac.shape[1]
...
adata = sca.models.organize_multiome_anndatas(
    adatas = [[rna_1, rna_2], [atac, None], [None, adt]],    # a list of anndata objects per modality, RNA-seq always goes first
    layers = [['counts', 'counts'], ['log-norm', None], [None, 'clr']], # if need to use data from .layers, if None use .X
)
del rna_1, rna_2, atac, adt
